	...
	<div id="yui-main">
	   <div id="content" class="yui-b hfeed rounded">
				...
				<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
								...
								<h1 class="page-title"><a href="<?php echo get_permalink( $post->post_parent ); ?>" title="<?php esc_attr( printf( __( 'Return to %s', 'travelblogger' ), get_the_title( $post->post_parent ) ) ); ?>" rel="gallery"><?php
									/* translators: %s - title of parent post */
									printf( __( '<span class="meta-nav">&larr;</span> %s', 'travelblogger' ), get_the_title( $post->post_parent ) );
								?></a></h1>
								...
								<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
									...
									<div class="entry-meta">
										...
										<?php
											...
											if ( wp_attachment_is_image() ) {
												...
												printf( __( 'Full size is %s pixels', 'travelblogger'),
													sprintf( '<a href="%1$s" title="%2$s">%3$s &times; %4$s</a>',
														wp_get_attachment_url(),
														esc_attr( __('Link to full-size image', 'travelblogger') ),
														$metadata['width'],
														$metadata['height']
													)
												);
									...
									<div class="entry-content">
										...
										<p class="attachment"><a href="<?php echo $next_attachment_url; ?>" title="<?php echo esc_attr( get_the_title() ); ?>" rel="attachment"><?php
											$attachment_size = apply_filters( 'travelblogger_attachment_size', 900 );
