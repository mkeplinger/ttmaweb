	...
	*/
	...
	class YoutubeSidebarWidget extends WP_Widget {
		...
		function widget($args, $instance) {
			...
			$height = ($width * 3) / 4;
			$username_url = "http://gdata.youtube.com/feeds/base/users/$ident/uploads?alt=rss&amp;v=2&amp;orderby=published&amp;client=ytapi-youtube-profile";
			...
			$output = "<div id='youtube-sidebar-widget'>\n\r";
			$i = 1;
						...
						if($fil) {
							if(strpos($item->description, $fil)) {
								if($i <= $num) {
									...
									$hash = explode("&", $link[1]);
									$img = "http://i2.ytimg.com/vi/$hash[0]/0.jpg";

									if($titles == "yes") {
										$output .= "<li id='$hash[0]'><img src='$img' alt='$item->title' desc='" . strip_tags($item->description) . "' style='width:" . $width . "px;' /><div class='play_arrow' style='width:" . $width . "px; height: " . $height . "px; margin-top: -" . ($height + 10) . "px;'></div>\n\r<a href='$item->link'>$item->title</a>";
									} else {
										$output .= "<li id='$hash[0]'><img src='$img' alt='$item->title' desc='" . strip_tags($item->description) . "' style='width:" . $width . "px;' /><div class='play_arrow' style='width:" . $width . "px; height: " . $height . "px; margin-top: -" . ($height + 10) . "px;'></div>\n\r";
									}
					...
					foreach($simple_xml->entry as $item) {
						foreach($item->link as $link_url) {
							if($link_url['type'] == "text/html" && $link_url['rel'] == "alternate") {
								...
								if($titles == "yes") {
									$output .= "<li id='$hash[0]'><img src='$img' alt='$item->title' desc='$item->title' style='width:" . $width . "px;' /><div class='play_arrow' style='width:" . $width . "px; height: " . $height . "px; margin-top: -" . ($height + 10) . "px;'></div>\n\r<a href='#'>$item->title</a>";
								} else {
									$output .= "<li id='$hash[0]'><img src='$img' alt='$item->title' desc='$item->title' style='width:" . $width . "px;' /><div class='play_arrow' style='width:" . $width . "px; height: " . $height . "px; margin-top: -" . ($height + 10) . "px;'></div>\n\r";
								}
		...
		function form($instance) {				
			$instance = wp_parse_args( (array) $instance, array('title'=>'', 'ident_type'=>'username', 'ident'=>'', 'num_videos'=>'2', 'filter'=>'', 'width'=>'180') );
