...
class ttfImage {
	...
	*/
	public function __construct($text, $style, $displaytext, $gzip){
		...
		$stylesheet =  ANYFONT_FONTDIR."/styles.ini";
		if($this->style == "admin"){
	...
	private function textCairo(){
// 		$img = new CairoImageSurface(CairoFormat::ARGB32, 500, 180);
//
// 		$context = new CairoContext($img);
//
// // 		$context->save ();
// // 		$RGB = self::hexToRgb($this->fontsettings['background-color']);
// // 		$context->setSourceRgb ($RGB[0], $RGB[1], $RGB[2]); /* white */
// //
// // 		$context->paint ();
// //
// // 		$context->restore ();
//
// 		$FontRGB = self::hexToRgb($this->fontsettings['background-color']);
//
// 		$context->setSourceRgb ($FontRGB[0], $FontRGB[1], $FontRGB[2]); /* black */
//
//
//
// 		$context->selectFontFace (ANYFONT_FONTDIR."/".$this->fontsettings['font-name'].$this->fonttype/*, CairoFontSlant::NORMAL, CairoFontWeight::NORMAL*/);
//
// 		$context->setFontSize ($this->fontsettings['font-size']);
//
//
//
// 		$context->moveTo ( 0, $this->fontsettings['font-size']);
//
// 		$context->showText ($this->text);
//
// 		$img->writeToPng($this->cache_file);
// 		self::printImage();
	}
	...
	*/
	private function textImagick(){
		...
		if($this->fontsettings['shadow']){
			...
			$this->fontsettings['shadow-spread'] > 0 ? $image->blurImage(0, (int)$this->fontsettings['shadow-spread']) : 0;
		}
		if($this->linecount > 1 && $this->fontsettings["line-height"] > 0){
			$l = 0;
			$lines = explode("\n", $this->text);
